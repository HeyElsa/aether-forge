...
<video src="/videos/03-agent-generation.mp4" poster="/video-posters/03-agent-generation.jpg" aria-label="Aether Forge documentation video" controls playsInline style={{width:'100%',aspectRatio:'16/9',background:'#0a0a0a',borderRadius:12,marginTop:16,marginBottom:16,display:'block'}} preload="none" />
...
Key flags:
- `--planner-mode` — force a specific LLM provider (`ollama`, `anthropic`, `openai`, `gemini`, `openrouter`)
- `--planner-model` — specify the exact model name
...
forge run . --planner-mode anthropic --planner-model claude-sonnet-4.5

# Switch to local Ollama (free, private)
forge run . --planner-mode ollama --planner-model gemma4:latest

# Use any OpenRouter model
forge run . --planner-mode openrouter --planner-model meta-llama/llama-4-maverick
```
...
```bash
forge run ./sol-scalper --environment sandbox --mode paper \
...
```bash
forge run ./sol-scalper --environment production --mode live --chain base --a2a-port 9001
```
