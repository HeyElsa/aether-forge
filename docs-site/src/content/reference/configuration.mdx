...
```jsonc
{
  "planner": {
    ...
    "source": "autodetected",                 // or "explicit" when --planner-mode was passed
    "detectedAt": "2026-05-16T18:00:00+00:00"
  }
}
```

`forge doctor` surfaces three states: `explicit` → "production-safe"; `autodetected` → advisory to pin via `AETHER_FORGE_PLANNER_MODE` or `--planner-mode`; `unstamped` (legacy v0.20.0 configs) → advisory to regenerate.
