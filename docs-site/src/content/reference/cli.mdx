...
then continues from the hold point. Pair with `--approval-token` to
auto-approve a held step.

```bash
forge resume-replay ./my-agent/replays/tick_0008.json \
    --approval-token approver-cli-2026-05-02
```
...
```bash
forge x402-call --endpoint get-swap-quote \
    --payload '{"in":"USDC","out":"WETH","amount":"100"}' \
    --agent-dir ./my-agent --max-usd 0.05
```
...
forge models-list --provider anthropic
```
...
```bash
forge init                                # in cwd
forge init ./my-existing-project
```
