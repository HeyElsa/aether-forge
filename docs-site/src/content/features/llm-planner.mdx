...
<video src="/videos/04-llm-planner.mp4" poster="/video-posters/04-llm-planner.jpg" aria-label="Aether Forge documentation video" controls playsInline style={{width:'100%',aspectRatio:'16/9',background:'#0a0a0a',borderRadius:12,marginTop:16,marginBottom:16,display:'block'}} preload="none" />
...
| 5 | Ollama (local fallback) | `gemma4:latest` |
| 6 | Heuristic | Rule-based fallback |

The detected config is baked into `aether-forge.json` so anyone running the agent later gets the same model. Ollama is probed only when no cloud key is set, unless `AETHER_FORGE_ALLOW_OLLAMA_AUTODETECT=1` is set.
