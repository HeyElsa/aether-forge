...
Supersedes: `docs/prd/aether-forge-prd-v0.5.0.md`
Supporting research:

- `docs/plans/2026-04-06-aether-forge-research.md`
- `docs/plans/2026-04-06-aether-forge-prd-autoresearch.md`
- `docs/plans/2026-04-06-aether-forge-prd-exact-autoresearch.md`
...
This version was produced by a stricter exact-style autoresearch pass against the PRD itself. The kept improvements tighten the requirement contract in three narrow areas:

- artifact evolution and migration contracts
- credential-handle boundaries and trace-safe secret handling
- explicit effect semantics for side-effecting capabilities
...
8. `Protected evaluation contracts during optimization`
9. `Explicit artifact, credential, and effect contracts`
...
7. The system is hard to make durable across versions.
Without explicit compatibility, migration, secret-handling, and effect-semantic rules, a spec-first system can still become brittle in practice.
...
### 7.10 Canonical artifacts

Each major product concern should have a single canonical artifact owner so the system does not drift into duplicated and conflicting representations.

### 7.11 Credentials by handle, not by value

Secret-bearing material should stay out of specs, ordinary agent logic, prompts, persisted step state, and traces whenever logical handles can be used instead.

### 7.12 Declared effect semantics

Side-effecting capabilities should declare idempotency, retry, duplicate-submit, and compensation semantics explicitly instead of leaving them implicit in runtime behavior.

### 7.13 Self-evolution remains governed
...
- artifact compatibility and migration contracts
- handle-based credential references and explicit effect semantics metadata
...
- automatic spec mutation inferred from arbitrary scaffold edits
...
- baseline active comparison contract
...
- fixed allowed mutation surfaces for the active comparison cycle

### 10.4 Active Comparison Contract

At baseline creation, the system must snapshot an immutable active comparison contract containing:
...
- budget rules
- active artifact-set ID

If any of these change materially, a new comparison cycle with a new baseline must begin.

### 10.5 Keep-Or-Discard Decision Loop
...
### 10.6 Measurable Improvement Criteria
...
### 10.7 Minimum Evidence For `keep`

A candidate may be marked `keep` only if its ledger records:

- stated hypothesis
- changed surfaces
- comparison contract reference
- measured outcomes against declared criteria
- rationale for why gains are not explained solely by extra budget or extra complexity

### 10.8 Slow Mode Stop Rules
...
### 12.1 Artifact Set Contract

Every generation cycle should produce a coherent artifact set.

The system must assign a shared artifact-set ID and compatibility metadata linking each generated:

- `Agent Spec`
- `Capability Manifest`
- `Scenario Pack`
- scaffold version
- `Research Record` when present
- `Promotion Record`

Canonical ownership should be explicit:

- `Agent Spec` owns user intent, permissions, environment contract, and evaluation references
- `Capability Manifest` owns executable access inventory, provider constraints, credential-handle boundaries, effect semantics, and approval metadata
- `Scenario Pack` owns evaluation cases and thresholds
- scaffold metadata owns generated/user/protected boundaries and regeneration behavior
- `Research Record` owns evidence and iteration history
- `Promotion Record` owns decision and rollout evidence

If overlapping data appears elsewhere, the canonical artifact wins and derived copies must be identifiable as derived.

Every versioned artifact must declare a machine-readable compatibility status against its predecessor:
...
Any artifact with a `breaking` or `incompatible` schema change must include or reference a machine-readable migration contract describing:

- required transforms
- lossy fields
- validation checks

Each generated artifact set must also record:

- generator version
- input digest

So regeneration behavior remains attributable and comparison-safe.
...
- exportable in a stable machine-readable format
- separated from secrets

The `Agent Spec` must reference credentials only by logical credential IDs or handles.
...
- seed phrases
- private keys
- provider-issued raw credentials
...
For each capability requiring credentials, the `Capability Manifest` must also declare:

- required credential handle
- allowed environments
- maximum access scope
- rotation or expiry expectation

For every side-effecting capability, the `Capability Manifest` must declare machine-readable effect semantics including:

- idempotency class
- duplicate-submit handling behavior
- retry policy
- compensation class

Minimum idempotency classes must include:

- `idempotent`
- `conditionally idempotent`
- `non-idempotent`

Minimum compensation classes must include:

- `revertible`
- `compensatable`
- `irreversible`

The `Capability Manifest` should also be exportable in a stable machine-readable format.
...
The scaffold must include machine-readable ownership metadata identifying:

- generated files or regions
- user-owned files or regions
- protected files or regions for the active comparison cycle

The scaffold must also define explicit regeneration modes:

- `safe update` for generated-only surfaces
- `propose patch` for overlapping surfaces
- `blocked` when user-owned edits prevent a safe merge

Within an active comparison cycle, the `Agent Spec` remains the canonical source for generated scaffold surfaces.
...
The `Scenario Pack` should also be exportable in a stable machine-readable format.

### 12.7 Runtime Session
...
### 12.8 Promotion Record

The evidence package behind environment promotion.

It should include:

- artifact-set ID and artifact versions
- scenario results
- policy outcomes
- offline and online evaluation results
- structured promotion decision outcome
- residual risk statement
- approver metadata
- rollout stage and status
- rollout limits
- expiry or review time when applicable
...
Runtime events, policy decisions, scenario results, and promotion evidence must each be tagged as:

- exactly replayable
- approximately replayable
- audit-only
...
### 16.5 Availability, Credential Scope, And Effect Linkage

If the policy engine, policy bundle, or required approval service is unavailable, side-effecting actions must fail closed unless an explicitly pre-approved emergency procedure exists for that environment.

Every governed action attempt must emit a policy decision record with, at minimum:
...
Policy enforcement must validate credential-handle scope against the requested action and environment before any side-effecting call is attempted.

Effect records must reference the policy decision ID used to authorize or deny them.
...
V1 drift monitoring should define at least these monitored surfaces:

- execution drift
- dependency drift
- policy drift
- domain-metric drift
- model-behavior drift where measurable

### 18.3 Stage Outcome Semantics

Each evaluation stage must produce a structured outcome of:
...
Each outcome should include blocking reason IDs and waiver expiry where applicable.

### 18.4 Scenario requirements
...
### 18.5 Protected Evaluation Surface
...
### 19.2 Promotion evidence and decision
...
Promotion evidence must include a structured promotion decision describing:
...
### 19.3 Rollout limits and automatic holds

Canary and production rollout plans must express machine-enforceable limits such as:

- max capital exposure
- max order notional
- allowed venues or accounts
- allowed chains or contracts
- execution rate caps
- rollout review window

The framework must support policy-configured automatic rollout halt conditions based on threshold breaches such as:
...
### 19.4 Runtime safety operations
...
For each promotable version, the rollback model must also record:

- which surfaces are revertible
- the fallback target version
- compensation requirements for irreversible side effects
- post-rollback verification checks required before resume

Compensation requirements for compensatable or irreversible side effects must reference the compensation class declared in the `Capability Manifest`.

### 19.6 Incident lifecycle

Incident workflows must support at least:

- severity classification
- declared incident state
- owner assignment
- affected environments
- resume authorization rules

Minimum incident states should include:
...
Any resolved incident caused by agent behavior, dependency behavior, policy gap, or rollout failure must either add or explicitly decline an incident-derived regression scenario with rationale.

### 19.7 Independent review
...
For each active comparison cycle, allowed mutation surfaces must be enumerated in machine-readable form and mapped to concrete artifacts or paths.

Any mutation outside that set must be rejected or treated as the start of a new cycle.
...
- The system must validate the spec and cross-artifact consistency.
- The system must distinguish validation classes at minimum as schema validation, referential validation, policy-surface validation, environment-availability validation, and interoperability-import validation.
- Validation results must return structured error codes, artifact references, severity, and remediation hints.
- The system must separate secrets from specs.
- The system must generate a `Capability Manifest` and `Scenario Pack`.
- The system must assign a shared artifact-set ID and compatibility metadata across generated artifacts.
- The system must classify artifact version changes by compatibility type and reject unresolved cross-artifact incompatibilities.
- The system must support validation of declared migration contracts before promotion or regeneration across artifact versions.
- Each generated artifact set must record generator version and input digest.
- The system must support stable machine-readable export for `Agent Spec`, `Capability Manifest`, and `Scenario Pack`.
...
- Slow mode must snapshot an immutable active comparison contract for the cycle.
...
- Slow mode may mark a candidate `keep` only when minimum evidence for improvement is recorded.
...
- The scaffold must represent generated, user-owned, and protected zones in machine-readable metadata.
...
- The system must support regeneration modes of `safe update`, `propose patch`, and `blocked`.
- The `Agent Spec` must remain the canonical source for generated scaffold surfaces during an active comparison cycle.
...
- The runtime must persist a step ledger that records input state snapshot reference, planned action, pre-action policy decision, approval outcome if required, effect execution result, post-action invariant result, and resulting state transition.
- The runtime must resolve credentials only through environment-scoped, time-bounded handles at execution time.
- The runtime must not expose raw secret material to ordinary agent logic, prompts, traces, or persisted step state.
...
- Side-effecting actions must fail closed when policy or approval dependencies are unavailable unless an explicitly approved emergency procedure exists.
- Every governed action attempt must have a policy decision record linked to the effect record.
- Policy enforcement must validate credential-handle scope against the requested action and environment before side-effecting execution.
...
- The system must record and enforce declared idempotency class, duplicate-submit handling behavior, retry policy, and compensation class for side-effecting capabilities, with duplicate-submit protection where supported by the provider.
...
- Each evaluation stage must produce a structured outcome of `pass`, `pass with waiver`, `hold`, or `fail`.
...
- The system must support machine-readable rollout limits and automatic hold conditions.
- The system must support rollback target selection and post-rollback verification.
- The system must support incident states, severity classes, owner assignment, and resume authorization rules.
- The system must support incident-to-regression closure.
...
- Imported external descriptions must not replace the internal canonical `Agent Spec` or `Capability Manifest`.
- The system must preserve a record of unsupported or lossy mappings when importing supported external standards.
- For any officially supported external standard, the system must define which fields are preserved exactly, normalized, or dropped.
...
- attributable artifact generation across versions
...
It must also expose derived health signals and alerts for at least:

- policy-deny rate
- approval wait time
- execution success and failure rate
- drift and anomaly rate
- rollout-stage health
- incident-triggered deactivations

The evidence model must support referential linkage across:
...
Traces and logs must support deterministic redaction of secret-bearing fields and must record credential-handle references without recording recoverable secret material.
...
- unresolved migration contract blocks regeneration or promotion
- credential-handle scope mismatch blocks side effect
- non-idempotent capability is prevented from unsafe retry behavior
...
- automatic canary hold on threshold breach
- rollback to fallback version followed by verification checks
- incident closure that must add or decline a regression case
...
- percentage of rollouts halted automatically before broader blast radius
- recoverable secret leakage incidents in traces or persisted state
- unsafe retry or duplicate-submit attempts blocked before side effect
...
Goal:
Prove that ideas can become typed specs and credible scaffolds quickly, with explicit artifact ownership, compatibility, and validation.
...
Goal:
Ship executable policy semantics plus realistic evaluation, protected evaluators, and scenario infrastructure.

### Milestone 4: Crypto Starter Pack

Goal:
Ship wallet, exchange, onchain, data, credential-bound access, and compliance-aware crypto primitives.

### Milestone 5: Promotion And Ops

Goal:
Ship staged promotion, shadow/canary support, rollout controls, approvals, and incident controls.
...
- validation classes and outputs
- fast mode
- scaffold generator
- initial runtime
...
- credential-handle boundary enforcement
- exchange and onchain modules
- venue capability model
- effect semantics metadata enforcement
- paper and canary infrastructure
- rollout limits and automatic hold controls
...
7. Artifact migration or credential handling rules remain underspecified in implementation despite being present in the PRD.

8. The product sounds like a generic agent builder instead of a governed agent engineering system.
...
- Which mutation surfaces should be allowed by default in v1 slow mode?
- Which rollout limits are mandatory for every canary live deployment?
- Which artifact migrations should be supported automatically versus only validated and blocked?
...
The immutable contract that fixes evaluator version, policy thresholds, mutation surfaces, budget rules, and artifact context for an active candidate comparison cycle.

### Credential Handle

The non-secret logical reference used to resolve environment-scoped credentials at execution time.

### Effect Semantics

The declared idempotency, retry, duplicate-submit, and compensation behavior for a side-effecting capability.
