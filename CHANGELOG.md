...
After this release, all five frictions are closed end-to-end.

### Added
- **Language-agnostic planner-output spec** — new `docs/specs/planner-output.md` (normative behavioral contract) and `src/aether_forge/schemas/runtime/planner-output.schema.json` (structural contract). Documents the recovery algorithm from v0.21.0 (`_extract_json`) as a versioned spec (v1.0.0). Both the Python reference (`aether_forge.planner._extract_json`) and the new TypeScript SDK conform.
- **Shared conformance fixtures** — `tests/fixtures/planner-outputs/` ships 13 baseline fixtures covering clean JSON, fenced (` ```json ` and bare ` ``` `), reasoning preamble, trailing prose, combined preamble+fence+prose, braces inside string literals, top-level array, plain refusal, mid-JSON truncation, whitespace-only, bare scalar (RFC 8259), empty-plan-distinct-from-parse-failure. The Python suite (`tests/test_planner_output_spec.py`) and the TS suite (`sdk-ts/test/conformance.test.ts`) both parametrize over the same JSON files. Adding a fixture is a one-step change.
- **`@aether-forge/sdk` v0.1.0** — new TypeScript SDK under `sdk-ts/`. Three layers:
  - **Generated types** for every JSON schema (19 schemas: 8 artifacts, 4 common, 7 runtime) bundled into `src/schemas/generated/index.ts`. Generated by `scripts/generate-schemas.ts` via `json-schema-to-typescript`; committed so npm consumers see types without building. CI verifies committed output matches a fresh regeneration.
  - **Ajv-backed validators** — one `validate*` function per artifact / runtime / common schema, plus `validateArtifactBundle` and `assertValid<T>`. Returns Result objects (`{ ok: true, value } | { ok: false, errors }`) or throws `ValidationError` carrying the ajv error array. All schemas are pre-registered on the ajv instance so cross-schema `$ref` URLs resolve without network fetches.
  - **`parsePlannerOutput`** — TypeScript reference implementation of the planner-output spec. Identical recovery behavior to the Python `_extract_json`, enforced by 13 conformance fixtures + 14 unit tests.
  - **Protocol interfaces** — `Planner`, `ExecutionRouter`, `MemoryStore`, `DataSource`, `PlanningModel`, `DelegatedSigner`, `StepProposal`, `ExecutionResult`, `RuntimeSession`, `SigningIntent`, etc. Interface-only — the tick loop deliberately stays Python-side until cross-language adoption justifies a port. Mirrors the Python Protocols 1:1.
  - **Error hierarchy** — `AetherForgeError`, `PlannerParseError`, `ValidationError`, `SchemaCompatError`. Every thrown error from the SDK extends `AetherForgeError` so consumers can `catch (error: unknown)` with one check.
- **`@aether-forge` npm scope reserved** — the sub-package layout (`@aether-forge/sdk/x402` is planned for v0.1.1) is documented in the SDK README.
- **Build tooling** — `tsup` (esm + cjs + .d.ts), `vitest`, `tsx`. Bun-friendly but works with npm / pnpm equally. Lockfile committed at `sdk-ts/bun.lockb` is gitignored in favor of `package-lock.json` (added later) — the published artifact is what matters.
- **CI: `.github/workflows/sdk-ts.yml`** — runs on any change to `sdk-ts/**`, `src/aether_forge/schemas/**`, `tests/fixtures/planner-outputs/**`, or `docs/specs/planner-output.md`. Verifies committed generated types match fresh regeneration; type-checks; builds; runs the full vitest suite including the cross-language conformance fixtures.
- **41 new tests** total: 15 Python (`tests/test_planner_output_spec.py` — 13 fixtures + 2 meta) and 37 TypeScript (`sdk-ts/test/parse.test.ts` 14 + `validate.test.ts` 9 + `conformance.test.ts` 14). All pass. Python suite: 620 → 635. TypeScript suite: 0 → 37.

### Notes
- **Back-compat**: zero impact on existing Python users. The new TS SDK lives in `sdk-ts/` as a sibling package; no Python code imports it. The new fixtures don't change Python parser behavior.
- **Schema bundle generation**: a deliberate trade-off. We emit a single `src/schemas/generated/index.ts` (~30 KB of types) rather than file-per-schema because cross-schema `$ref` resolution under file-per-schema produced duplicate type exports across files. Single bundle keeps the public surface clean (`import { AgentSpec } from "@aether-forge/sdk"`) at the cost of one larger file.
- **What's deferred**: the `@aether-forge/sdk/x402` sub-package (browser sign-and-relay for hosted marketplaces — closes FP-3 on the browser side) is scoped for v0.1.1 and explicitly out of v0.1.0. The Python-side `DelegatedSigner` Protocol from v0.22.0 is the contract the TS-side `eip1193Signer` will satisfy.
